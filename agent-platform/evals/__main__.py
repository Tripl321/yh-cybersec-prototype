...
Exit code 0 means every selected case passed, 1 means one failed, and 2 means the
selector matched nothing — so a mistyped `--tag` fails a CI gate rather than greening
it on an empty run. Both log to Postgres through `eval_db`.
"""
...
if __name__ == "__main__":
    sys.exit(cli(CASES, db=eval_db, judge_model=default_model()))
