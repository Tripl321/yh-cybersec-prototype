...
- Hooks: add `**BUILDER_HOOKS` on any case that can reach the builder's ungated
  ...
  learning-store component (`agno`, `platform-manager`, `platform-engineer`).
...
CASES: tuple[Case, ...] = (
    # Agno — capture: the fact lands in the entity graph (asserted by the outcome
    # teardown, which sweeps it) and the reply confirms it briefly (judge). No
    # expected_tool_calls: 8B Ollama models file under a stable near-miss alias, so
    # a name-exact reliability check fails a run that stored everything correctly.
    Case(
        ...
        teardown=cleanup_memory_landing,
        criteria=(
    ...
    # prose permission-ask first, and the build ends PUBLISHED. The snapshot-diff
    # teardown hard-deletes it after.
    Case(
    ...
    # must produce a grounded no, never a fabricated status.
    Case(
