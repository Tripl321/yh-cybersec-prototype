...
- **SHALLOT** — Proximity-baserat säkerhetssystem för OT-åtkomstkontroll.
...
- **Cub** — Fältnod / ID-bricka. Roll: `cub`.
...
| SHALLOT | WebAuthn |
| --- | --- |
| Mama Bear | user med roll `mama_bear` + credentials |
| Cub (ID-bricka) | user med roll `cub` + credential |
| Proximity-åtkomst | (framtida) heartbeat/Transit styr om sessionen ges auktorisation |
